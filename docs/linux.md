...
This guide covers the **v0.30.0-alpha.1** downloads for Ubuntu 24.04, Fedora 44
and current Arch Linux on **x86_64 / amd64** PCs. Use the
[published Alpha release](https://github.com/NekoUnix/A.R.I.A/releases/tag/v0.30.0-alpha.1).
...
| --- | --- | --- |
| Ubuntu 24.04 | `aria-0.30.0-alpha.1-linux-x64.tar.gz` | Included in the archive; install separately with its script |
| Fedora 44 | `aria-alpha-0.30.0.alpha.1-1.x86_64.rpm` | `aria-obs-canvas-0.30.0.alpha.1-1.x86_64.rpm` |
| Current Arch Linux | `aria-alpha-0.30.0alpha.1-1-x86_64.pkg.tar.zst` | `aria-obs-canvas-0.30.0alpha.1-1-x86_64.pkg.tar.zst` |
...
tar -xzf aria-0.30.0-alpha.1-linux-x64.tar.gz -C "$HOME/Applications"
"$HOME/Applications/aria-0.30.0-alpha.1-linux-x64/aria-cli" --version
"$HOME/Applications/aria-0.30.0-alpha.1-linux-x64/aria-desktop"
```

The CLI should print `aria-cli 0.30.0-alpha.1`. Keep the extracted files together.
...
sh "$HOME/Applications/aria-0.30.0-alpha.1-linux-x64/install-obs-linux.sh"
```
...
```sh
sudo dnf install ./aria-alpha-0.30.0.alpha.1-1.x86_64.rpm
aria-cli --version
aria-desktop
```
...
```sh
sudo dnf install ./aria-obs-canvas-0.30.0.alpha.1-1.x86_64.rpm
```
...
```sh
sudo pacman -Syu
sudo pacman -U ./aria-alpha-0.30.0alpha.1-1-x86_64.pkg.tar.zst
aria-cli --version
aria-desktop
```
...
```sh
sudo pacman -U ./aria-obs-canvas-0.30.0alpha.1-1-x86_64.pkg.tar.zst
```
