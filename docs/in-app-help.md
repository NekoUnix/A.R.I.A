...
VRM and GLB advance spring physics every rendered frame, using substeps no longer than 8.3 ms. Tracking poses are interpolated through the steps, drag follows elapsed time and strength blends the displayed bend once, so a weaker setting remains fluid at high refresh rates. Tiny links have bounded parent lag; generated contacts recover gradually with an angular-speed limit and retain sliding momentum instead of snapping or discarding all motion. Exported VRM groups and colliders are retained; missing breast/body, hair/tail/ear/clothing chains are guessed from the active skeleton. Tracking bones, fingers, facial controls and twist helpers retain their rig roles. Generated physics is Experimental and cannot recover Unity PhysBone settings. In Find secondary bones, open the searchable Bone inspector to see every imported skeleton bone and endpoint. Auto uses its guessed role; Simulate includes an eligible branch; Keep rigid excludes it and its children. Hover its status to see its parent, mesh influence and endpoint source. Unweighted end nodes define chain length, and weighted bones with no child can use a bounded mesh-derived virtual endpoint. Unknown names need review. Unrigged artwork cannot bend. Choices and group tuning save per avatar in profiles and presets.

Soft, Medium and Firm reset overall and group tuning while keeping detection/manual roots and generated collision settings. Medium uses 0.15 extra damping, a 45-degree bend limit and enabled collision response. Motion strength blends the simulated bend once (above one caps at full strength), without repeatedly damping the simulation. Lower strengths move the visible pose toward rest and can reduce its contact accuracy. Inertia carries momentum; response pulls back to rest. Gravity multiplies exported gravity, which can be zero. Wind adds a steady sideways force. Global and group multipliers combine. Higher extra damping settles sooner. Maximum bend bounds the joint relative to its posed parent. Per-group advanced settings inherit overall values until Custom is checked.

Very small rotations are retained so gentle idle/tracking can move both sides of a paired body rig. Each side still uses its own weights, collisions and tuning. If one side stays still, check both branches in the bone inspector for Keep rigid and both spring groups for disabled physics, zero strength or zero maximum bend. Use torso tracking or Life & idle movement to exercise body chains; head-only input mainly moves chains attached to the head. Medium resets group tuning but keeps bone overrides, so an excluded bone needs Auto separately. Settings remain saved per avatar.

Generated collision shapes resolve close contacts precisely without repeated outward nudges. This lets close-fitting straps and accessories settle while preserving small tracking movements and sliding. The shapes are still approximate envelopes, not a surface-level cloth solver.

Settle motion clears momentum. Reset group restores inheritance. Changes save automatically per avatar; Save physics for this avatar, Save profile and movement presets retain them too. Freeze stores the final joint rotations for screenshots. All OBS outputs share one simulation.

Body & self collision defaults on for generated physics. Body shapes are fitted to weighted head, torso and limb regions and follow the humanoid rig. Moving envelopes for other flexible groups keep breast/hair/accessory regions apart; related bones in one branch do not collide with themselves. Up to 256 flexible envelopes are retained, prioritizing large regions. Body collision size scales fitted body capsules; Spring thickness adds a radius as a fraction of bone length. Raise gently to reduce clipping, or lower if movement feels crowded. The panel shows fitted body shapes and contact corrections from the latest simulation update. Use collision shapes must also be enabled overall or in that group's override. If no body shapes are found, review the GLB humanoid assignments and skin weights. Authored VRM groups retain their supplied shapes.

Collision uses approximate bone envelopes, not triangle-mesh cloth simulation. It checks bone segments and fast tip crossings while preserving length/bend limits. Generated envelopes retain overlaps present in the current rigged pose, including relaxed arms and gestures, so they do not force clothing away from the body to recover the export's T-pose. All groups sample a consistent collider pose each physics step. Contacts constrain the simulated bend; strength below one blends the displayed pose toward rest and can reduce contact accuracy. Thin garments, regions outside fitted shapes and bones within one branch may still intersect. Spring collision does not reposition the rigid arms/hands driven by tracking or gestures; use Relax arms and gesture strength for those. Choices save with the avatar, profiles and presets.
...
Click Select layers… above Your stage or in the Layers inspector to open a separate frozen-model window. Select is the default: click artwork to select it, click again to deselect it, or drag several boxes to add layers without holding Shift. Selected visible artwork turns blue, and a gold mesh outline identifies the layer under the pointer. Choose Remove, Replace or Toggle for different mouse behavior, then Hide selected or Restore selected. The live model keeps tracking. Undo, zoom, pan, Refresh frozen pose and preview-only Reveal hidden layers help you refine the result. Open the question mark in that window for the full guide.
...
To protect artwork from selection, enable Protect from selection in its saved group settings. Include protected layers allows deliberate selection; switch it off to exclude protected members again. Protection saves per avatar and does not disable opacity controls or visibility hotkeys.
...
Click Select layers… above Your stage or in Avatar → Layers. A resizable window shows a frozen copy of the current pose. Your main model keeps tracking. The preview shares artwork with the loaded model and renders only when layer settings or selection change; it does not load another Core or decode the atlas again.

Select is the default mouse mode. Click a visible layer to select it, then click it again to deselect it. Each click toggles only the frontmost eligible mesh. Drag one box after another to add layers without holding any keyboard key. Shift-click explicitly adds. Remove subtracts layers with clicks or boxes; Replace starts a fresh selection; Toggle switches membership. A click picks the frontmost exported mesh. Esc cancels the current drag. Selected visible artwork is tinted blue using its atlas transparency and clipping. A gold outline follows the hovered mesh perimeter; hover to read its ID, selection state and click action. Hidden or covered selected layers remain in the list. Blue colors never change your saved tints or enter OBS/PNG output.
...
Enter a group name and choose Save selection as group to reuse the selection. Check Protect this new group before saving to exclude its members from future clicks, boxes, list ranges and Select results. This does not hide them. Selection protection in the frozen window, or Protect from selection in the inspector group settings, changes this setting for existing groups. Include protected layers deliberately allows picking protected members; switching it off removes those members from the selection. Any protected overlapping group is sufficient. Protection saves per avatar and with preset layer groups, independent of group visibility and its hotkey. Edit selected layers in group settings enables the override so you can revise all members. Direct opacity controls, Show all, saved looks and visibility hotkeys still work: this protects selection, not every edit. The new group starts inactive; Manage groups & hotkeys opens the Layers inspector where you can toggle it or assign a shortcut. Selection uses exported mesh triangles, which can include transparent texture regions or clipped areas; uncheck unwanted IDs in the selection list. Original PSD layers that were merged or omitted cannot be recovered. The window and its selection guides never enter OBS or PNG exports.
...
Open Inspector → Avatar → View. Life & idle movement enables a subtle body sway, chest breathing and relaxed arm/elbow motion. Each strength runs from zero to two; zero gently stops that component. Idle speed runs from 0.25× to 2×. Strength, speed and on/off edits blend smoothly (about half a second for most of the change) so menu adjustments do not kick clothing into motion. These motions add to tracking in avatar-facing axes and feed the model's spring-bone physics. Generated collision clearance follows the current rigged pose, including relaxed arms, so it does not try to force clothing back into the export's T-pose. Tune Relax arms in Pose controls for its resting arm angle. Chest motion falls back to the spine when the optional chest bone is absent.
...
## vbridger | Experimental VBridger configuration import | Experimental compatibility: import and edit VBridger tracking equations for this avatar.

Open Tracking → Import / edit VBridger config, or Inspector → Tracking → Inputs → VBridger config & equations. Choose a .vbridger file and review every imported output. Expand a row to see its equation, range and model assignments. Apply saves the supported configuration to the current model. Cancel or closing the window discards the draft. Unsupported fields, functions and dependencies appear in the notices; they never execute as scripts.

The importer reads encoded and plain V2 files, older encoded JSON files, and ARIA's complete JSON exports. Vector outputs become separately editable X, Y and Z channels. Identical duplicate rows are merged with a notice; conflicting duplicates reject the file. Missing VTS assignments can be restored from the model's .vtube.json. Existing customized mappings keep their authored output ranges. Unassigned channels remain available in the model's Source menu.

Imported outputs replace ARIA's built-in derived tracking, gains, mirroring and personal calibration for their matching channels. Imported head inputs use separate pitch, yaw and roll axes and the current neutral head origin. Correct directions using the imported pitch/yaw/roll switches. Model mapping, expression, physics, pose/freeze and responsive speech controls still apply. The ordinary range wizard opens this editor while imported tracking is enabled; disable the import to use the ordinary wizard.

Enable imported tracking toggles this avatar's saved setup. Edit saved settings makes a draft; Apply commits it. Undo last import restores the complete previous rig configuration. Movement presets include the imported equations, input curves, offsets and modifiers and can use the existing preset hotkeys. Export complete config creates portable ARIA JSON with all settings. Export .vbridger outputs only writes V2 equations, output curves and modifiers, omitting ARIA input calibration and global settings. Vector channels are exported as scalar outputs.

Live values and missing-input diagnostics help check the tracker. A microphone provides volume; viseme channels need an external phoneme tracker. Native VMC transport and automatic VRM bone routing are not included. Raw inputs and available devices differ across tracking sources. Config compatibility is an Alpha implementation, with timing/filter differences described in the editor help.

## vbridger-editor | Equation and modifier editor | Tune expressions, response curves, delays, smoothing and stepped movement.

Each output has an Enabled switch, a Requires a tracked face switch, a numeric equation, minimum/default/maximum values and collapsible modifiers. Edit supported mathematical functions, comparison operators, boolean expressions and lazy if branches. Output dependencies are sorted at Apply; unknown inputs, cycles and invalid ranges block the edit. Invalid numerical results use the output default. The VBridger faceSend flag is preserved for export as metadata; it does not configure ARIA's network routing.

Processing order: input offsets and input curves → equation → output curve and range → delay → smoothing → steps → avatar input mapping. Delay holds a bounded history for up to five seconds. Smoothing is lerp with a 0–1 retention amount, adjusted for elapsed time relative to 60 Hz; zero follows immediately and one holds its starting value. This approximates VBridger's documented smoothing; exact private filter behavior is not guaranteed. Responsive mouth smoothing can add another short filter after the imported output.

Steps have Trigger, Target, Threshold and Hold ms. Trigger is the entry value, Target is the held output, Threshold is the absolute falling boundary, and Hold is the minimum duration before another change. Imported V2 thresholds are absolute, not distances. The importer adds missing range endpoint steps where appropriate. Use the five-step preset as a starting point, then change targets and hold times. Sort triggers before applying.

Output curves support cubic tangents, weighted handles and Clamp, Loop or Ping-pong outside their keys. Drag pink points or edit their coordinates, tangents and weights. Linear and Smooth S presets replace the current curve; Copy/Paste lets another curve reuse it. Clear means identity. Keys must have increasing times. Only visible expanded curves are drawn.

The time(increment, maximum) function advances at a simulated 60 Hz rate; rand(low, high) uses repeatable 60 Hz noise. Timing and random sequences can differ from VBridger. Expressions are bounded numeric data: no file access, code loading, shell commands or network calls. Imports allow at most 2 MiB, 256 expanded outputs, 64 curve keys and 64 steps per output. History is local to the running model and is reset by changing configs or presets.

## vbridger-inputs | Imported input curves and calibration | Match raw blendshapes to your face before equations run.

Expand Input calibration and curves, then a raw input. An offset adds to that input before its response curve. Use current face as blendshape zero captures offsets for the referenced facial blendshapes; hold a relaxed, visible face. Reset offsets clears those corrections. Head centering still uses ARIA's ordinary neutral head calibration. Linear and Smooth S create editable curves; drag points, edit values/tangents/weights or copy a curve to its opposite-side input. Reset input curves clears this avatar's curves.

VBridger normally saves input curves separately from .vbridger output files. ARIA cannot recover data that is absent from a supplied export. Its complete JSON export includes input curves and offsets alongside outputs, so those settings can be shared and reimported together. Original standalone VBridger input-curve file formats have not been validated.
...
## model-mount | Mount two Live2D models | Choose a point on each model to join them while both receive tracking.

Open Stage → Objects on your Live2D avatar and choose Add & mount Live2D. Select the second model's .model3.json (recommended, with its textures and physics) or .moc3. The current avatar stays loaded. A new independent object appears and the mount window opens. For an existing Live2D object, select it and choose Mount two models.

### Pick both points

Click the desired mesh in the current-avatar preview, then click the desired mesh in the added-model preview. Yellow crosshairs show the two points. Click again or drag to reposition either point before applying. Choose Mount together: the added model moves so its selected point touches the current avatar's selected point. This resets the object's fine offset to zero; size and rotation are retained. Cancel leaves an existing mount unchanged; a newly added object remains available in the object list.

    Current avatar mesh point ● = ● Added model mesh point
                     follows tracking on both sides

The previews are live. Freeze the pose first if you want still models while choosing points. Both points follow their own animated mesh triangles, so the connection stays together as tracking or physics deforms either model. Picking uses mesh triangles, not texture alpha or clipping-mask pixels; choose a nearby triangle if the selected part is not the one you intended.

### Tracking and appearance

Send current tracking to the added model is enabled for new objects. Each model receives the same active tracking inputs through its own imported or inferred mappings and its own physics. Models with different rigs may react differently; an input needs a matching mapping to drive a parameter. Object parameter overrides take precedence. Turn tracking off to use a static child pose; Freeze pose holds both models. Tracking settings on the main avatar do not overwrite the child's mappings.
