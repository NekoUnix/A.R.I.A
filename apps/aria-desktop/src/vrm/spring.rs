//! Fixed-step VRM spring joints with sphere/capsule collisions and center space.
    ...
    pub fn update(
        ...
        if self.tails.len() != groups.len() {
            self.tails = groups
                .iter()
                .map(|g| {
                    let inv = g.center.map_or(Mat4::IDENTITY, |c| world[c].inverse());
                    g.joints
                        .iter()
                        .map(|j| {
                            let p = inv.transform_point3(world[j.node].transform_point3(j.tip));
                            Tail {
                                previous: p,
                                current: p,
        ...
        while self.accumulator >= step {
            ...
            for (g, tails) in groups.iter().zip(&mut self.tails) {
                ...
                for (j, tail) in g.joints.iter().zip(tails) {
                    ...
                    if !tuning.enabled {
                        tail.current = inverse.transform_point3(target);
                    ...
                    let current = center.transform_point3(tail.current);
                    let previous = center.transform_point3(tail.previous);
                    let inertia = (1. - j.drag).clamp(0., 0.99) * settings.inertia * tuning.inertia;
                    let force = j.stiffness * settings.response * tuning.response;
                    let wind = Vec3::X * (settings.wind + tuning.wind) * 0.5;
                    let mut next = current
                        + (current - previous) * inertia.min(0.995)
                        + axis / length * force * step
                        + (j.gravity * j.power * settings.gravity * tuning.gravity + wind) * step;
                    next =
                    ...
                    tail.previous = tail.current;
                    tail.current = inverse.transform_point3(next);
        ...
        // Preserve the latest fixed-step pose on displays refreshing faster than 60 Hz.
        for (g, tails) in groups.iter().zip(&self.tails) {
            for (j, tail) in g.joints.iter().zip(tails) {
                rotations[j.node] = tail.rotation;
            }
